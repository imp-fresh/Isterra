...
const CATEGORIES = [
  {key:'tops',      label:'Tops',      img:'img/catalog/p01.jpg'},
  {key:'sombreros', label:'Sombreros', img:'img/catalog/p02.jpg'},
  {key:'bolsos',    label:'Bolsos',    img:'img/catalog/p04.jpg'},
  {key:'chalecos',  label:'Chalecos',  img:'img/catalog/p07.jpg'},
  {key:'hogar',     label:'Hogar',     img:'img/catalog/p14.jpg'},
];
...
function filterCat(cat, btn){
  ...
  if(bc){
    if(cat==='todos'){ bc.style.display='none'; }
    else {
      bc.style.display='inline';
      const label = CATEGORIES.find(c=>c.key===cat)?.label || cat;
      bc.innerHTML = `<i class="ti ti-chevron-right"></i><span>${label}</span>`;
    }
  }
}
...
function goToCategory(key){
  ...
  setTimeout(()=>{
    const btn = Array.from(document.querySelectorAll('.tab')).find(t => t.dataset.cat===key);
    if(btn) filterCat(key, btn);
  }, 350);
}
